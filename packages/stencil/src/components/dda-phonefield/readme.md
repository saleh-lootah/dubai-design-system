...
| `autocomplete`               | `autocomplete`               | `autocomplete` token of the phone input. Defaults to `tel-national`, because the country code is chosen separately.                                       | `string`  | `'tel-national'`        |
| `button_aria_label`          | `button_aria_label`          | Accessible name set on each country option button in the open list.                                                                                       | `string`  | `undefined`             |
| `button_id`                  | `button_id`                  | Prefix of the country option button ids in the open list. Each option gets `<button_id>-<index>`. When it is not set, the prefix comes from the input id. | `string`  | `undefined`             |
...
| `input_id`                   | `input_id`                   | `id` of the phone input. Also used for the label `for` and the helper and error text ids. When it is not set, the component generates a unique id.        | `string`  | `undefined`             |
