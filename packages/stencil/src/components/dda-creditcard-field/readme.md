# dda-creditcard-field



<!-- Auto Generated Below -->


## Properties

| Property          | Attribute         | Description | Type      | Default       |
| ----------------- | ----------------- | ----------- | --------- | ------------- |
| `aria_label`      | `aria_label`      |             | `string`  | `undefined`   |
| `autocomplete`    | `autocomplete`    |             | `string`  | `'cc-number'` |
| `card_icon`       | `card_icon`       |             | `string`  | `undefined`   |
| `component_mode`  | `component_mode`  |             | `string`  | `undefined`   |
| `custom_class`    | `custom_class`    |             | `string`  | `''`          |
| `disabled`        | `disabled`        |             | `boolean` | `false`       |
| `error_message`   | `error_message`   |             | `string`  | `undefined`   |
| `helper_text`     | `helper_text`     |             | `string`  | `undefined`   |
| `input_id`        | `input_id`        |             | `string`  | `undefined`   |
| `input_name`      | `input_name`      |             | `string`  | `undefined`   |
| `input_type`      | `input_type`      |             | `string`  | `undefined`   |
| `label`           | `label`           |             | `string`  | `undefined`   |
| `placeholder`     | `placeholder`     |             | `string`  | `undefined`   |
| `restrict_input`  | `restrict_input`  |             | `boolean` | `false`       |
| `size`            | `size`            |             | `string`  | `undefined`   |
| `validation_type` | `validation_type` |             | `string`  | `undefined`   |
| `value`           | `value`           |             | `string`  | `''`          |


----------------------------------------------

*Built with [StencilJS](https://stenciljs.com/)*
